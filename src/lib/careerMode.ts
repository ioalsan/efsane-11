...
const migrateCareerSave = (save: CareerSave): CareerSave => ({
  ...
  history: save.history.map((entry) => {
    const legacy = entry as Partial<CareerHistoryEntry>;
    return {
      season: legacy.season ?? 1,
      teamName: legacy.teamName ?? save.club.teamName,
      leaguePosition: legacy.leaguePosition ?? 18,
      ...
      trophies: legacy.trophies ?? [],
      bestPlayerName: legacy.bestPlayerName ?? '-',
      topScorerName: legacy.topScorerName ?? '-',
      topScorerGoals: legacy.topScorerGoals ?? 0,
      careerPointsGained: legacy.careerPointsGained ?? 0,
      boardGrade: legacy.boardGrade ?? '-',
      fanGrade: legacy.fanGrade ?? '-',
      note: legacy.note ?? 'Sezon tamamlandı',
    };
  }),
